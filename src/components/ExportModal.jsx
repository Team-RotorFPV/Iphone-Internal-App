  ...
  const OptionCard = ({ active, onClick, icon, label, sub }) => (
    <button type="button" className={`opt-card${active ? ' active' : ''}`} onClick={onClick}>
      <div className="opt-head">
        <div className="opt-iconbox" style={{ background: active ? '#FF980020' : 'var(--elevated)' }}>
          {icon}
        </div>
        {active ? <CheckSquare size={18} color="#FF9800" /> : <Square size={18} color="var(--text-muted)" />}
      </div>
    ...
    >
        ...
        <div className="opt-grid">
          {scopes.map((s) => (
            <OptionCard
              ...
              icon={<Layers size={18} color={scope === s.key ? '#FF9800' : 'var(--text-secondary)'} />}
              label={s.label}
            />
          ))}
        </div>
      ...
      <AppSection title="File Format" style={{ marginTop: 18 }}>
        <div className="opt-grid">
          <OptionCard
            ...
            icon={<FileText size={18} color={format === 'csv' ? '#FF9800' : 'var(--text-secondary)'} />}
          ...
          <OptionCard
            ...
            icon={<Table size={18} color={format === 'excel' ? '#FF9800' : 'var(--text-secondary)'} />}
      ...
      {result && (
        <div className={`result-box ${result.type}`}>
          {result.type === 'success' ? (
            <CheckCircle2 size={18} color="#66BB6A" />
          ) : (
