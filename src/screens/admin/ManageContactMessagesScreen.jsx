    ...
    <Screen title="Manage Messages">
      ...
      {loading ? (
        ...
        filteredMessages.map((item) => {
          ...
          return (
            <AppCard key={item.id} variant={isUnread ? 'elevated' : 'surface'} style={isUnread ? { borderColor: 'rgba(139,92,246,0.5)' } : undefined}>
              <div className="row-between" style={{ alignItems: 'flex-start', marginBottom: 14 }}>
                <div className="row gap-md grow" style={{ marginRight: 12 }}>
                  <div className="icon-well" style={{ width: 40, height: 40, borderRadius: 12, background: isUnread ? '#8B5CF620' : '#A855F715' }}>
              ...
              <div className="wrap gap-sm" style={{ marginBottom: 14 }}>
                {item.queryType && (
                  <span className="badge" style={{ background: '#38BDF815', color: '#38BDF8', borderColor: '#38BDF830' }}>
              ...
              <div className="row gap-sm">
                ...
                <AppButton variant="danger" size="sm" onClick={() => handleDelete(item)} icon={<Trash2 size={16} color="#EF4444" />} />
              </div>
            </AppCard>
          );
        })
